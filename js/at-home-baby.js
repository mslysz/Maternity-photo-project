export default function initAtHomeBaby() {
  ...
  // CAROUSEL SLIDESHOW
  const carousel = el('.at-home-baby-slideshow__carousel');
  const btnPrev = el('.at-home-baby-slideshow__btn--prev');
  const btnNext = el('.at-home-baby-slideshow__btn--next');
  ...
  // ACCORDION FAQ
  const faqContainer = el('.at-home-baby-questions');

  if (faqContainer) {
    faqContainer.addEventListener('click', (e) => {
      const button = e.target.closest('.at-home-baby-questions__header');
      if (!button) return;

      const parent = button.closest('.at-home-baby-questions__question');
      const content = parent.querySelector('.at-home-baby-questions__content');
      const isExpanded = button.getAttribute('aria-expanded') === 'true';

      allEl('.at-home-baby-questions__question').forEach((item) => {
        if (item !== parent) {
          item.classList.remove('at-home-baby-questions__question--active');

          const header = item.querySelector('.at-home-baby-questions__header');
          if (header) header.setAttribute('aria-expanded', 'false');

          const itemContent = item.querySelector(
            '.at-home-baby-questions__content',
          );
          if (itemContent) {
      ...
      if (isExpanded) {
        parent.classList.remove('at-home-baby-questions__question--active');
        button.setAttribute('aria-expanded', 'false');
        content.style.maxHeight = '0px';
      } else {
        parent.classList.add('at-home-baby-questions__question--active');
